...
def preprocess_data(xFeat: pd.DataFrame, y: pd.DataFrame):
    ...
    df: pandas df with shape (n, d)
    """
    xFeat = xFeat[["timestamp_local", "rh", "temp", "pm25", "pm10"]].dropna(subset=["pm25"])
    ...
    xFeat['pm25_roll_3h'] = xFeat['pm25'].rolling(window=3, min_periods=1).mean()
    xFeat['pm25_roll_6h'] = xFeat['pm25'].rolling(window=6, min_periods=1).mean()
    xFeat['pm25_std_3h'] = xFeat['pm25'].rolling(window=3, min_periods=1).std().fillna(0)

    # Interaction terms - these capture how RH/temp affect bias differently at different PM levels
    xFeat['pm25_x_rh'] = xFeat['pm25'] * xFeat['rh']
    xFeat['pm25_x_temp'] = xFeat['pm25'] * xFeat['temp']
    xFeat['rh_x_temp'] = xFeat['rh'] * xFeat['temp']
    
    # Polynomial terms - capture non-linear effects
    xFeat['pm25_sq'] = xFeat['pm25'] ** 2
    ...
    # y
    y.dropna(subset=["pm25"], inplace=True)
