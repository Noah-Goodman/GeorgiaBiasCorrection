...
def preprocess_data(xFeat: pd.DataFrame, y: pd.DataFrame):
    ...
    df: pandas df with shape (n, d)
    """
    xFeat = xFeat[["timestamp_local", "rh", "temp", "no"]].dropna(subset=["no"])
    ...
    xFeat['no_roll_3h'] = xFeat['no'].rolling(window=3, min_periods=1).mean()
    xFeat['no_roll_6h'] = xFeat['no'].rolling(window=6, min_periods=1).mean()
    xFeat['no_std_3h'] = xFeat['no'].rolling(window=3, min_periods=1).std().fillna(0)

    # Interaction terms - these capture how RH/temp affect bias differently at different PM levels
    xFeat['no_x_rh'] = xFeat['no'] * xFeat['rh']
    xFeat['no_x_temp'] = xFeat['no'] * xFeat['temp']
    xFeat['rh_x_temp'] = xFeat['rh'] * xFeat['temp']
    
    # Polynomial terms - capture non-linear effects
    xFeat['no_sq'] = xFeat['no'] ** 2
    ...
    # y
    y.dropna(subset=["no"], inplace=True)
