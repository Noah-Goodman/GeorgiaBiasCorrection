...
def main():
    ...
    xFeat = pd.read_csv(f"../.data/Lithonia/collocation/raw.csv")
    y = pd.read_csv(f"../.data/Lithonia/collocation/gapa-{args.species}.csv").dropna(subset=[args.species])

    xFeat, y, scaler = preprocess_data(xFeat, y, args.species)

    # update gapa csv file to only have target species values
    y.to_csv(f"../.data/Lithonia/collocation/gapa-{args.species}.csv", index=False)

    # Save preprocessed data to be used for visualization and dayhour alignment
    xFeat.to_csv(f"../.data/Lithonia/collocation/preprocessed-{args.species}.csv")

    joblib.dump(scaler, f"scalers/rh-temp-scaler-{args.species}.pkl")

    xTrain, xTest, yTrain, yTest = holdout(xFeat, y)
    xTrain.to_csv(f"../.data/Lithonia/split/xTrain-{args.species}.csv", index=False)
    xTest.to_csv(f"../.data/Lithonia/split/xTest-{args.species}.csv", index=False)
    yTrain.to_csv(f"../.data/Lithonia/split/yTrain-{args.species}.csv", index=False)
    yTest.to_csv(f"../.data/Lithonia/split/yTest-{args.species}.csv", index=False)
